var Boot = {
    globals: {
      ...
      block: {
          fieldError: function(){
              DialogBox.setWith('ion-android-warning', 'Veuillez remplir les champs correctement !', 'ok').show();
          }
      }
    },
    terminal: {
        setAYUI: function(current){
            if(current != null){
                $('.academic .years .empty-info').addClass('not-super');
        ...
        setStudentUI : function(form,data){
            ...
            form.find('h1').text(data == undefined ? 'Nouvel Élève' : 'Élève');
        ...
        fac: function(){
            ...
            $('.administration')
                ...
                .on('click', '#addLevel', function(){
                    FormPopup.onAction(function(submit, data, required){
                            if(submit){
                                if(Form.isSubmitable(data, required)){
                                    ...
                                    Fetch.post('./submit', extend(data,{
                                        ...
                                        currentGradeId = null;
                                        FormPopup.hide();
                                    }).catch(Boot.globals.callbacks.error);
                                }
                                else{
                                    DialogBox.setWith('ion-android-warning', 'Veuillez remplir les champs correctement !', 'ok').show();
                                }
                ...
                .on('click', '.administration .add-class', function(){
                    var lvl_id = $(this).attr('lvl_id');
                    FormPopup.onAction(function(submit,data,required){
                        console.log('[SUBMIT]',{data,required})
                        if(submit){
                            if(Form.isSubmitable(data,required)){
                                DialogBox.setWith('ion-load-d animate-spin', 'Requête en cours...', 'none')
                                    .show();
                                Fetch.post('./submit',extend(data,{
                                    ...
                                    // DialogBox.setWith('ion-android-done green', 'Classe enregistrée', 'ok');
                                    // currentViewData = Boot.terminal.updateCurrentFacData(currentViewData);
                                    // Boot.terminal.setFacUI(currentViewData);
                                    History.reach();
                                    $('.administration .card-tab').eq(1).trigger('click');
