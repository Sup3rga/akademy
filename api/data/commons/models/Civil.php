...
abstract class Civil extends Data {
    ...
    private array $criteria = [
        "nom"=> "civil.lastname.required",
        "prenom"=> "civil.firstname.required"
    ];
    ...
    public function setCriterias(array $criterias){
        foreach ($criterias as $index=>$message){
            $this->criteria[$index] = $message;
        }
        return $this;
    }

    public function __search(string $attr, object $target = null){
        $r = [
          "found"=>false,
          "value"=>null
        ];
        $target = $target ? $target : $this;
        foreach ($target as $attr_name => $value){
            if($attr_name == $attr){
                $r["found"] = true;
                $r["value"] = $value;
                break;
    ...
    public function save($target = null, $extras = null){
        $target = $target ? $target : $this;
        $r = _str["civil.lastname.incorrect"];
        if(!CheckIf::isFormalName($this->nom)) return $r;
        $r = _str["civil.firstname.incorrect"];
        if(!CheckIf::isFormalName($this->prenom)) return $r;
        $r = _str["civil.code.incorrect"];
        if($this->code && !CheckIf::isCode($this->code)) return $r;
        $r = _str["civil.code.exist"];
        if($this->code && self::codeExists($this)) return $r;
        $r = _str["civil.gender.invalid"];
        if ($this->sexe && !$this->sexe == "M" && $this->sexe != "F") return $r;
        $r = _str["civil.address.format.incorrect"];
        if ($this->adresse && !CheckIf::isFormalAddress($this->adresse)) return $r;
        $r = _str["civil.birthplace.format.incorrect"];
        if ($this->lieu_naissance && !CheckIf::isFormalPlace($this->lieu_naissance)) return $r;
        $r = _str["civil.birthdate.format.incorrect"];
        if ($this->date_naissance && !CheckIf::isDate($this->date_naissance)) return $r;
        $r = _str["civil.phone.format.incorrect"];
        if ($this->telephone && !CheckIf::isPhoneNumber($this->telephone)) return $r;
        $r = _str["civil.email.format.incorrect"];
        if ($this->email && !CheckIf::isEmail($this->email)) return $r;
        $r = _str["civil.email.exist"];
        if ($this->email && self::emailExists($this)) return $r;
        $r = _str["civil.nif.format.incorrect"];
        if ($this->nif && !CheckIf::isNIF($this->nif)) return $r;
        $r = _str["civil.nif.exist"];
        if ($this->nif && self::nifExists($this)) return $r;
        $r = _str["civil.ninu.format.incorrect"];
        if ($this->ninu && !CheckIf::isNINU($this->ninu)) return $r;
        $r = _str["civil.ninu.exist"];
        if ($this->ninu && self::ninuExists($this)) return $r;
        if($this->_poste > 0 && Hierarchy::isSaturated($this->_poste, $this->identite)){
            return _str["civil.hierarchy.saturated"];
        }
        foreach($this->criteria as $field => $message){
            $attr = $this->__search($field, $target);
            if($attr["found"] && !$attr["value"]){
                return _str[$message];
            }
        }
        $this->setAvatar();
        ...
        if($this->identite == 0){
            $db = $db->prepare("insert into individu (code,nom,prenom,sexe,adresse,email,lieu_naissance,
                date_naissance,nif,ninu,poste,telephone,photo,memo,version) 
                values(:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,:p12,:p13,:p14,:p16)");
        }
        else{
            $db = $db->prepare("update individu set code=:p1,nom=:p2,prenom=:p3,sexe=:p4,adresse=:p5,email=:p6,
                ...
                photo=:p13,memo=:p14,version=:p16 where id=:p15");
            $data["p15"] = $this->identite;
            \System\Log::println("[Date] Update " . $this->date_naissance);
        }
        $data["p1"] = $this->code;
        $data["p2"] = $this->nom;
        $data["p3"] = $this->prenom;
