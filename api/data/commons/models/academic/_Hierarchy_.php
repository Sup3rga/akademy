...
abstract class _Hierarchy_ extends Data
{
    protected $notation, $filiere;
    protected $capacity = 0, $value = 0, $affectation = 0, $id = 0;
    public static $list = [];
    ...
    public static function fetchAll($override = false){
        ...
        $db = Storage::Connect();
        $chk = $db->prepare("select distinct h.*, f.nom as filiere from hierarchie h, filieres f 
                        where h.version > :p98 and (h.affectation is null or h.affectation = f.id) order by h.id asc");
        $doublon = [];
        try{
            $chk->execute([
