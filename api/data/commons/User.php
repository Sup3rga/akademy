...
class User extends Civil
{
    ...
    public function save($password = null, $teacher = null){
        if($teacher != null){
    ...
    public static function isMatch(string $user, string $passcode, bool $checkOnly = false){
        ...
        try{
            ...
            if($chk->rowCount()){
                ...
                if(!$checkOnly) {
                    if (!$r->isActif()) {
                        throw new Exception(_str["user.locked"]);
                    }
                }
            }
        }catch(Exception $e){\System\Log::printStackTrace($e);}
    ...
    public static function fetchAll($override = false){
        ...
        self::$list = [];
        $db = Storage::Connect();
        $chk = $db->prepare("select u.* from utilisateurs u, individu i where i.id = u.identite and u.version > :p98 order by i.prenom, i.nom asc");
        try{
    ...
    public static function pseudoExists(User $e){
        ...
        foreach(self::fetchAll() as $u){
            if($u->getId() != $e->getId() && strtolower($u->getPseudo()) == strtolower($e->getPseudo()) ){
                $r = true;
                break;
            }
        }
